...
export default function ReadModelComponet({ navigation, route }) {
  //    = route.model;
  const { user, model, component } = route.params; // Get the user data passed from UserDetails screen
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        let modeldata = await get_data(model, user);
